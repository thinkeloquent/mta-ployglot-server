...
def discover_files(directory: str, suffixes: Tuple[str, ...]) -> DiscoverResult:
    ...
    # (e.g. ".lifecycle.py"). Anything else — helper modules (`_<name>.py`),
    # `__pycache__/`, dotfiles, unrelated source — is dropped silently. It is
    # not the discoverer's job to flag unrelated files; the addon's later
    ...
    for name in os.listdir(directory):
        ...
        if name.endswith(suffixes):
            matched.append(full)
    return DiscoverResult(matched=sort_by_numeric_prefix(matched))
